...
void print_matrix(double *data, int width, int start, int end)
{
    int i, j;
    for (j=start; j<end; j++) {
        for (i=0; i<width; i++) {
...
void fprint_matrix(double *data, int width, int start, int end, FILE *file)
{
    int i, j;
    for (j=start; j<end; j++) {
        for (i=0; i<width; i++) {
            int index = j * width + i;
			/* Check whether file reader needs int */ 
            fprintf(file, "%f ", data[index]);
        }
        fprintf(file, "\n");
    }
}
