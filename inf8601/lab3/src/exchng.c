...
void exchng1d_async(ctx_t *ctx) {
	...
	double *offset_recv_1 = data + (row_end - 1) * width;
	double *offset_recv_2 = data + row_start * width;
	MPI_Irecv(offset_recv_1, width, MPI_DOUBLE, south, 0, comm1d, &req[1]);
	MPI_Irecv(offset_recv_2, width, MPI_DOUBLE, north, 1, comm1d, &req[0]);

	double *offset_send_1 = data + (row_end - 2) * width;
	double *offset_send_2 = data + (row_start + 1) * width;
	MPI_Isend(offset_send_1, width, MPI_DOUBLE, south, 1, comm1d, &req[3]);
	MPI_Isend(offset_send_2, width, MPI_DOUBLE, north, 0, comm1d, &req[2]);
...
void exchng1d_buffered(ctx_t *ctx) {
	...
	double *offset_send = data + (row_end - 2) * width;
	double *offset_recv = data + row_start * width;
	MPI_Bsend(offset_send, width, MPI_DOUBLE, south, 0, comm1d);
	MPI_Recv(offset_recv, width, MPI_DOUBLE, north, 0, comm1d, &status);
	if (ctx->verbose) {
	...
	MPI_Bsend(offset_send, width, MPI_DOUBLE, north, 1, comm1d);
	MPI_Recv(offset_recv, width, MPI_DOUBLE, south, 1, comm1d, &status);
	if (ctx->verbose) {
		fprintf(ctx->log, "after exchange south->north\n");
		fprint_matrix(data, width, row_start, row_end, ctx->log);
	}
...
void exchng1d_combined(ctx_t *ctx) {
	...
	double *offset_send = data + (row_end - 2) * width;
	double *offset_recv = data + row_start * width;
	MPI_Sendrecv(offset_send, width, MPI_DOUBLE, south, 0, offset_recv, width, MPI_DOUBLE, north, 0, comm1d, &status);
	if (ctx->verbose) {
		fprintf(ctx->log, "after exchange north->south\n");
		fprint_matrix(data, width, row_start, row_end, ctx->log);
	}
	offset_send = data + (row_start + 1) * width;
	offset_recv = data + (row_end - 1) * width;
	MPI_Sendrecv(offset_send, width, MPI_DOUBLE, north, 0, offset_recv, width, MPI_DOUBLE, south, 0, comm1d, &status);
	if (ctx->verbose) {
		fprintf(ctx->log, "after exchange south->north\n");
		fprint_matrix(data, width, row_start, row_end, ctx->log);
	}
...
void exchng1d_ordered(ctx_t *ctx) {
	...
	if ((ctx->coords[0] % 2) == 0) {
		double *offset_send = data + (row_end - 2) * width;
		double *offset_recv = data + row_start * width;
		MPI_Send(offset_send, width, MPI_DOUBLE, south, 0, comm1d);
		MPI_Recv(offset_recv, width, MPI_DOUBLE, north, 0, comm1d, &status);
		if (ctx->verbose) {
			fprintf(ctx->log, "after exchange north->south\n");
			fprint_matrix(data, width, row_start, row_end, ctx->log);
		...
		offset_send = data + (row_start + 1) * width;
		offset_recv = data + (row_end - 1) * width;
		MPI_Send(offset_send, width, MPI_DOUBLE, north, 1, comm1d);
		MPI_Recv(offset_recv, width, MPI_DOUBLE, south, 1, comm1d, &status);
		if (ctx->verbose) {
			fprintf(ctx->log, "after exchange south->north\n");
			fprint_matrix(data, width, row_start, row_end, ctx->log);
