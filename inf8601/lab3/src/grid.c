...
grid_t *make_grid(int width, int height, int padding) {
	...
	grid->data = (double *) calloc(count, sizeof(double));
	if (grid->data == NULL)
		goto error;
	grid->height = height;
	grid->width = width;
...
void grid_max(grid_t *grid, double *value) {
	...
	for (j = 0; j < g.ph; j++) {
		for (i = 0; i < g.pw; i++) {
			double val = g.data[IX2(i,j,g.pw)];
			if (val > max)
				max = val;
		}
	}
...
void grid_copy(grid_t *src, grid_t *dst) {
	...
	for (j1 = s.padding, j2 = d.padding; j1 < s.height + s.padding && j2 < d.height + d.padding; j1++, j2++) {
		for (i1 = s.padding, i2 = d.padding; i1 < s.width + s.padding && i2 < d.width + d.padding; i1++, i2++) {
			d.data[IX2(i2,j2,d.pw)] = s.data[IX2(i1,j1,s.pw)];
		}
	}
}
...
void grid_set_min(grid_t *min, grid_t *dst) {
	...
	for (j1 = s.padding, j2 = d.padding; j1 < s.height + s.padding && j2 < d.height + d.padding; j1++, j2++) {
		for (i1 = s.padding, i2 = d.padding; i1 < s.width + s.padding && i2 < d.width + d.padding; i1++, i2++) {
			...
			double act = d.data[dst_index];
			double desired = s.data[src_index];
			if (act < desired) {
				d.data[dst_index] = desired;
			}
		}
	}
}
...
void grid_set_bounds1(grid_t *grid) {
	...
	int h = g.ph;
	for (i = 1; i < w - 1; i++) {
		g.data[IX2(i,0,w)] = g.data[IX2(i,1,w)];
		g.data[IX2(i,h-1,w)] = g.data[IX2(i,h-2,w)];
	}
	for (j = 1; j < h - 1; j++) {
		g.data[IX2(0,j,w)] = g.data[IX2(1,j,w)];
		g.data[IX2(w-1,j,w)] = g.data[IX2(w-2,j,w)];
	}
	// corners
	g.data[IX2(0,0,w)] = g.data[IX2(1,1,w)];
	g.data[IX2(w-1,0,w)] = g.data[IX2(w-2,1,w)];
	g.data[IX2(0,h-1,w)] = g.data[IX2(1,h-2,w)];
	g.data[IX2(w-1,h-1,w)] = g.data[IX2(w-2,h-2,w)];
}
