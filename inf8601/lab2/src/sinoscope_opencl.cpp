...
int create_buffer(int width, int height)
{
     ...
     */
    output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, height * width * sizeof(rgb), NULL, &ret);
    ERR_THROW(CL_SUCCESS, ret, "CL buffer could not be created.");
done:
    return ret;
error:
    ret = -1;
...
int sinoscope_image_opencl(sinoscope_t *ptr)
{
    ...
    cl_int ret = 0;
    cl_event ev;
    size_t work_range[2] = {(size_t)ptr->height, (size_t)ptr->width};

    if (ptr == NULL)
        goto error;

    ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), &output);
    ERR_THROW(CL_SUCCESS, ret, "Error setting the output kernel parameter");
    ...
    ERR_THROW(CL_SUCCESS, ret, "Error setting the sinoscope kernel parameter");

    ret = clEnqueueNDRangeKernel(queue, kernel, 2, 0, work_range, nullptr, 0, nullptr, nullptr);
    ERR_THROW(CL_SUCCESS, ret, "Error setting the work ragnge for the kernel");

    ret = clFinish(queue);
    ERR_THROW(CL_SUCCESS, ret, "Error during the finishing phase");

    ret = clEnqueueReadBuffer(queue, output, CL_TRUE, 0, ptr->height * ptr->width * sizeof(rgb), ptr->buf, 0, nullptr, nullptr);
    ERR_THROW(CL_SUCCESS, ret, "Error during the reading from the device buffer");
