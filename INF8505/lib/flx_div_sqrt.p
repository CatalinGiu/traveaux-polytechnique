...
-- File : flx_div_sqrt.p
--
-- Contents : FP division and square root
--
...
void fdiv_pre(w32 opa, w32 opb, w32& dvdnd, w32& dvsor, w32& other)
{
    ...
    uint24_t s_fracta_24 = (~s_opa_dn)::s_fracta; // naming copied from VHDL; this really is a significand
    uint24_t s_fractb_24 = (~s_opb_dn)::s_fractb; // naming copied from VHDL; this really is a significand

    s_expa = s_expa + s_opa_dn;
    s_expb = s_expb + s_opb_dn;

    // count leading zeros
    uint5_t s_dvd_zeros = count_l_zeros_u24( s_fracta_24 );
    uint5_t s_div_zeros = count_l_zeros_u24( s_fractb_24 );

    // left-shift the dividend and divisor, to obtained normalized number with 1 extra exp bit
    // also zero-extend up to width of 27 bits; quotient needs extra precision bits because of
    // post-normalizing right shift
    uint27_t s_dvsor_27 = (s_fractb_24 << s_div_zeros)::"000";
    uint27_t s_dvdnd_27 = "0"::(s_fracta_24 << s_dvd_zeros)::"00"; // right shift by 1 here
    ...
    tmp_other[22] = (opa[30:0]==0);
    tmp_other[21] = (opa[22:0]!=0);
    tmp_other[20:16] = s_dvd_zeros;
    
    tmp_other[15] = s_signb_i;
    tmp_other[14:7] = s_expb;
    tmp_other[6] = (opb[30:0]==0);
    tmp_other[5] = (opb[22:0]!=0);
    tmp_other[4:0] = s_div_zeros;
    
    other = tmp_other;

    FLT_DEBUG_PRINT("fdiv_pre: s_dvdnd_27=0x%x s_dvsor_27=0x%x other=0x%x", s_dvdnd_27, s_dvsor_27, tmp_other);
}

#if 0 // can share dstep instead
void dstep_27(w32 pi, w32 ai, w32& po, w32& ao, w32 b)
{
    uint28_t pi_28 = pi[27:0]; // extra msb required to avoid bits shifting out
    uint28_t ai_28 = ai[27:0];
    uint28_t b_28 = b[27:0];
    
    uint56_t pa = pi_28::ai_28;
    uint56_t new_pa = pa << 1;
    //FLT_DEBUG_PRINT("new_pa=%x",new_pa);
    uint29_t diff = (uint29_t)(new_pa[55:28]) - b_28;
    //FLT_DEBUG_PRINT("diff=%x",diff);
    if (diff[28] == 0) {
        new_pa[55:28] = diff;
        new_pa[0] = 1;
    }
    po =  (uint28_t)new_pa[55:28];
    ao =  (uint28_t)new_pa[27:0];
}
#endif

void dstep(w32 pi, w32 ai, w32& po, w32& ao, w32 b)
{
    uint64_t pa = pi::ai;
...
{  
    ...
    uint1_t fraca_nz = other[21]; //(opa[22:0]!=0);
    uint1_t fracb_nz = other[5];  //(opb[22:0]!=0);
    ...
    uint27_t s_qutnt_i = qutnt;
    uint27_t s_rmndr_i = rmndr;
    
    uint1_t s_qutdn = ~(s_qutnt_i[26]); // denormal quotient
    ...
    // decrement exponent if subnormal
    uint9_t s_expo2 = s_expo1 - (1-s_fraco1[26]); //s_fraco1[26] ? s_expo1 : s_expo1 - 1; 
    
    FLT_DEBUG_PRINT("fdiv_post s_fraco1=0x%x expo2=0x%x", s_fraco1, s_expo2);
 
    uint5_t s_r_zeros = count_r_zeros_u28(s_qutnt_i);
    ...
    uint25_t s_frac_rnd =  s_fraco1[26:3] + s_roundup;
    
    // Could be that after rounding, a subnormal becomes normal again
    s_expo2 = s_frac_rnd[23] & !(s_fraco1[26]) ? 1 : s_expo2;
              // normal        // subnormal
 
    // post-normalize after rounding
    uint1_t s_shr2 = s_frac_rnd[24];
    
    uint9_t s_expo3   = s_shr2 ? s_expo2 + 1      : s_expo2;
    uint25_t s_fraco2 = s_shr2 ? s_frac_rnd[24:1] : s_frac_rnd;
    ...
    if (s_nan_in | s_nan_op) {
        ...
        s_output_o = s_sign_i::ZERO_VECTOR; //"0"::ZERO_VECTOR;
    }
    else {
        s_output_o = s_sign_i::s_expo3[7:0]::s_fraco2[22:0];
    }
...
void fsqrt_pre(w32 opa, w32& remainder, w32& other)
{
    ...
    // count leading zeros
    uint5_t s_rdcnd_zeros = count_l_zeros_u24( s_fracta_24 );
    ...
    //// post-normalizing right shift
    uint24_t s_rdcnd_24 = (s_fracta_24 << s_rdcnd_zeros);
    //uint27_t s_dvsor_27 = (s_fractb_24 << s_div_zeros)::"000";
    //uint27_t s_dvdnd_27 = "0"::(s_fracta_24 << s_dvd_zeros)::"00"; // right shift by 1 here
                                                                   // left shift by 27 is app layer (dvdend in PH, instead of PL)
                                                                   // total left shift by 26, so binary point of quotient
    ...
    uint8_t s_exp_o = (opa[30:0] == 0) ? 0 : (uint8_t) s_exp_tem[8:1]; // s_exp_tem / 2 - bias present once
    uint25_t s_rdcnd_25 = s_exp_tem[0] ? (s_rdcnd_24::"0") : ("0"::s_rdcnd_24); // significand eats up s_exp_tem[0]
                                                                       // so in range [1,4)

    int25_t s_remainder = (int25_t) s_rdcnd_25 - (int25_t) 0x800000;
    remainder = (uint25_t) s_remainder;
    ...
    tmp_other[30] = (opa[22:0]!=0);
    tmp_other[29] = (opa[30:0]!=0);
    tmp_other[28] = (s_expa==0xff);
    
    other = tmp_other;

    FLT_DEBUG_PRINT("fsqrt_pre: s_rdcnd_25=0x%x s_remainder=0x%x, other=0x%x", 
                                    s_rdcnd_25::"0", s_remainder::"0", tmp_other);
...
{
    uint26_t s = s_in; // three whole bits, 23 fraction bits
    uint5_t index = q_in[31:27];
    uint25_t q = q_in[24:0]; // one whole bit, 24 fraction bits (extra fraction bit for rounding)
    
    uint5_t index_min_1 = index - (uint5_t)1;
    uint26_t shifted_01 = (uint25_t)"01" << index; // first index should be 23
    uint26_t subtrahend = (q::"0") | shifted_01;
    uint27_t trial_subtraction = (uint27_t) (s::"00") - (uint27_t) subtrahend;
                                // 3 whole bits (including sign), 24 fraction bits
                                // s is left-shifted by one, and receives an extra fraction bit
    uint1_t sign = trial_subtraction[26];
    ...
    uint26_t new_s = sign ? s[24:0]::"0" : (uint26_t) trial_subtraction[26:1];
    uint1_t new_q_bit = !sign;
    uint25_t q_bit_shifted = (uint25_t)new_q_bit << index;
    uint25_t new_q = q | q_bit_shifted;
    
    s_out = new_s;
    q_out = index_min_1::"00"::new_q;
...
w32 fsqrt_post(w32 q_in, w32 other)
{
    ...
    uint24_t s_frac_rnd = s_roundup ? (q + 1) : q;
    ...
    if (s_nan_a | s_nan_op) {
        ...
        s_output_o = s_sign::QNAN;
    }
    else if (s_infa) {
        FLT_DEBUG_PRINT("fsqrt_post INF");
        s_output_o = s_sign::INF;
    }
    else {
        s_output_o = s_sign::s_expo::s_frac_rnd[22:0];
    }
...
//#define Q_INIT ((1 << 24) | (23 << 27))
#define Q_INIT 0b10111::0b001_0000_0000_0000_0000_0000_0000
// initial q = 1.0 with 24 fractional bits + index 23 in bits [31:27]
...
multicycle_fu div 
{
    ...
    void process () {
        ...
        // Floating point division
            ...
            // one iterative division step
            w32 ah = dvdnd;
            w32 al = 0;
            //dstep(ah, al, AH, AL, dvsor);
            AH = ah;
            AL = al;
            ...
            // one final step
            //dstep(AH, AL, remainder, quotient, B);
            remainder = AH;
            quotient = AL;
        ...
        // Floating point square root
            ...
            // one iterative sqrt step
            // restoring binary sqrt on 25-bit integer with two whole bits and 23 fraction bits
            // compute root with one whole bit and 24 fraction bits (extra fraction bit needed for rounding)
            w32 ah = remainder;
            w32 al = q;
            ...
            // one final step
            //sqrt_step(AH, AL, remainder, quotient);
            w32 quotient = AL;
